	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if (compoundnbt != null) {
			...
			if (compoundnbt.contains("Age")) {
				...
				if (baby) tooltip.add((new TranslationTextComponent("tooltip.buzzier_bees.is_baby").applyTextStyles(atextformatting)));
			}
			if (compoundnbt.contains("Anger")) {
				boolean angry = compoundnbt.getInt("Anger") > 0;
				if (angry) tooltip.add((new TranslationTextComponent("tooltip.buzzier_bees.is_angry").applyTextStyles(atextformatting)));
			}
			if (compoundnbt.contains("HasNectar")) {
				boolean nectar = compoundnbt.getBoolean("HasNectar");
				if (nectar) tooltip.add((new TranslationTextComponent("tooltip.buzzier_bees.has_nectar").applyTextStyles(atextformatting)));
			}
			if (compoundnbt.contains("HasStung")) {
				boolean stung = compoundnbt.getBoolean("HasStung");
				if (stung) tooltip.add((new TranslationTextComponent("tooltip.buzzier_bees.has_stung").applyTextStyles(atextformatting)));
			}
		}
	}
}
